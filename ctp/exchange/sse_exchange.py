...
from ctp.sse.market_data import MarketData
from ctp.sse.trader import Trader
...
class SSEExchange(Exchange, ABC):
    def __init__(self, config):
        super().__init__(config)
        print(f'CTP API 版本: {ThostFtdcApiSOpt.CThostFtdcTraderApi_GetApiVersion()}')
        if not os.path.exists("../con_file/sse"):
            os.makedirs("../con_file/sse")

    def connect_market_data(self):
        print("连接上交行情中心")
        # 创建API实例
        self.market_data_user_api = ThostFtdcApiSOpt.CThostFtdcMdApi_CreateFtdcMdApi("../con_file/sse")
        # 创建spi实例
    ...
    def connect_trader(self):
        print("连接上交交易中心")
        self.trader_user_api = ThostFtdcApiSOpt.CThostFtdcTraderApi_CreateFtdcTraderApi("../con_file/cffex")
    ...
    def insert_order(self, code: str, direction: Direction, price, volume, strategy_id=0):
        ...
        if ret == 0:
            print('发送下单请求成功！')
            self.cffex_trader_user_spi.order_map[str(order_ref)] = OrderInfo(order_ref, strategy_id, self.trader_user_spi.front_id, self.trader_user_spi.session_id)
            # 报单回报里的报单价格和品种数据不对，所以自己记录数据
            self.cffex_trader_user_spi.order_map[str(order_ref)].order_price = price
            self.cffex_trader_user_spi.order_map[str(order_ref)].instrument_id = code
            print(self.cffex_trader_user_spi.order_map[str(order_ref)])
        else:
            print('发送下单请求失败！')
            judge_ret(ret)
